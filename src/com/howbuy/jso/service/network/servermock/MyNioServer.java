...
public class MyNioServer
{
    ...
    private void doRead(SelectionKey key)
    {
        try
        {
            ...
            while (stream.hasRemaining())
            {
                ...
                //打印客户端发来的请求流..
                //printStream(packData, false);
                
                CmdReq req = CmdReq.fromStream(packData);
                switch (CmdEnum.forCommand(req.getCmd()))
                {
                    case HEART_BEAT:
                        System.out.println("成功收到心跳请求from:[" + req.getSeqId() + "]");
                        onHeartBeatRsp(key, req.getSeqId());
                        break;
                    case CLEAR_ACK:
                        System.out.println("成功收到clear-ack请求from:[" + req.getSeqId() + "]");
                        onClearAck(key, req.getSeqId());
                        break;
                        
                    case CLEAR_DIS_ACK:
                        System.out.println("成功收到clear-dis-ack请求from:[" + req.getSeqId() + "]");
                        onClearDisAck(key, req.getSeqId());
                        break;
                        
                    case CLEAR_DIS_ACK_DTL:
                        System.out.println("成功收到clear-dis-ack-dtl请求from:[" + req.getSeqId() + "]");
    ...
    private void printStream(byte[] data, boolean isServer)
    {
        ...
        System.out.println(isServer ? "[server write:]"+ builder.toString() : "[read from client]:"+ builder.toString());
    }
